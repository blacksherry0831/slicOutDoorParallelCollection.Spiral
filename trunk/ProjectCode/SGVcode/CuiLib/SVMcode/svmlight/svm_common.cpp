...
SVECTOR *create_svector(WORDSV *words,char *userdefined,double factor)
{
  SVECTOR *vec;
  long    fnum,i;
  ...
  vec->words = (WORDSV *)my_malloc(sizeof(WORDSV)*(fnum));
  for(i=0;i<fnum;i++) { 
      vec->words[i]=words[i];
  }
	...
	SVECTOR is used */
{
    ...
    sum=(WORDSV *)my_malloc(sizeof(WORDSV)*veclength);
    sumi=sum;
    ai=a->words;
    bj=b->words;
    while (ai->wnum && bj->wnum) {
...
MODEL *read_model(char *modelfile)
{
  ...
  words = (WORDSV *)my_malloc(sizeof(WORDSV)*(max_words+10));
		    ...
		    long int *totwords, long int *totdoc)
{
  ...
  words = (WORDSV *)my_malloc(sizeof(WORDSV)*(max_words_doc+10));
  if(verbosity>=1) {
...
#if ProgressBar_InSvmlight

						CuiTrainHOG::ThreadData.pctrlProgress->SetPos(cui_t_i++);
...
int parse_document(char *line, WORDSV *words, double *label,
